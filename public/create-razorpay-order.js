...
module.exports = async function handler(req, res) {
  ...
  const aliases = { tp1: 'template01', tp2: 'template02', 'islamic-invitation': 'template01', 'islamic-invitation-premium': 'template02' };
  const reverseAliases = { template01: ['tp1', 'islamic-invitation'], template02: ['tp2', 'islamic-invitation-premium'] };
  const templateId = aliases[req.body.templateId] || req.body.templateId;
  ...
  try {
    ...
    if (templateId) {
      const idsToTry = [templateId, ...(reverseAliases[templateId] || [])].filter(Boolean);
      const { data: rows, error } = await sb
        .from('template_configs')
        .select('id, price, is_active')
        .in('id', idsToTry);
      const tpl = (rows || []).find(r => r.id === templateId) || (rows || [])[0];
